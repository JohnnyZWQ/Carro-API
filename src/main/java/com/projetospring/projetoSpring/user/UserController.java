...
public class UserController {
    ...
    public List<UserModel> listarCursos() {
        List<UserModel> usuarioCad = userRepository.findAll(); // Busca todos os usuários no banco.
        return usuarioCad;
    }
    ...
    public ResponseEntity atualizaUser(@RequestBody UserModel userModel) {
        var criado = this.userRepository.save(userModel); // Salva as alterações no banco.
        return ResponseEntity.status(HttpStatus.CREATED).body(criado); // Retorna status de criação com os dados atualizados.
    }
